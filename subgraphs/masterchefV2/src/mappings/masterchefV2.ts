...
export function logUpdatePool(event: LogUpdatePool): void {
  log.info('[MasterChefV2] Log Update Pool {} {} {} {}', [
    ...
    event.params.accSuniPerShare.toString()
  ])

  const masterChef = getMasterChef(event.block)
  const pool = getPool(event.params.pid, event.block)

  pool.accSuniPerShare = event.params.accSuniPerShare
...
export function deposit(event: Deposit): void {
  ...
  user.rewardDebt = user.rewardDebt.plus(event.params.amount.times(pool.accSuniPerShare).div(ACC_SUNI_PRECISION))
  user.save()
}
...
export function withdraw(event: Withdraw): void {
  ...
  user.rewardDebt = user.rewardDebt.minus(event.params.amount.times(pool.accSuniPerShare).div(ACC_SUNI_PRECISION))
  user.save()
}
...
export function harvest(event: Harvest): void {
  ...
  let accumulatedSushi = user.amount.times(pool.accSuniPerShare).div(ACC_SUNI_PRECISION)
